...
const NavBar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white fixed bg-black px-4 z-10'>
        <div>
            <h1 className='text-5xl font-signature ml-2'>Oleg</h1>
        </div>
        ...
        {nav? <FaTimes size={30}/>: <FaBars size={30}/>}
          ...
          { nav && (
            <ul className='flex flex-col justify-center absolute top-0 w-screen h-screen items-center bg-gradient-to-b from-black to-gray-800 text-gray-500'>
               { links.map(({id,link}) => (
